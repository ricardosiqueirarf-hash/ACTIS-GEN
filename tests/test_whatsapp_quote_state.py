...
def test_quote_payload_carries_mode_and_order_reference():
    quote = {"fields": {"glass": "esp prata"}, "quote_mode": "alteration", "quote_phase": "technical", "order_ref": "510", "revision": 2}
    ...
    assert payload["quote_phase"] == "technical"
    assert payload["order_ref"] == "510"


def test_quote_phase_only_moves_to_technical_after_explicit_close_or_production_signal():
    prior = {"quote_phase": "commercial", "status": "collecting", "quote_mode": "new"}
    assert qs.detect_quote_phase("Qual o valor?", prior) == "commercial"
    assert qs.detect_quote_phase("Pode fazer o orçamento?", prior) == "commercial"
    assert qs.detect_quote_phase("Aprovado, pode produzir", prior) == "technical"
    assert qs.detect_quote_phase("Vamos fechar e mandar para produção", prior) == "technical"


def test_new_quote_resets_technical_phase_to_commercial(monkeypatch):
    current = {
        "fields": {"typology": "giro", "height_mm": 800, "width_mm": 300},
        "status": "quoted", "quote_mode": "new", "quote_phase": "technical",
        "pending_confirmation": None, "revision": 5,
    }
    monkeypatch.setattr(qs, "get_quote_state", lambda agent_id, contact: dict(current))
    monkeypatch.setattr(qs, "save_quote_state", lambda agent_id, contact, quote: dict(quote))
    result = qs.update_quote_state("wa", "op", "Novo orçamento: duas portas 070 bronze 1000x400")
    assert result["quote_phase"] == "commercial"


def test_technical_missing_fields_do_not_reuse_commercial_defaults_as_confirmation():
    quote = {
        "quote_phase": "technical",
        "fields": {
            "typology": "giro", "height_mm": 800, "width_mm": 300,
            "profile_code": "1036", "color": "prata", "glass": "esp prata",
        },
    }
    missing = qs.quote_missing_fields(quote)
    assert "quantidade de dobradiças" in missing
    assert "lado das dobradiças" in missing
    assert "alturas das dobradiças" in missing
    assert "posição das furações" in missing
    assert "puxador/sem puxador" in missing
