...
def _current_organization() -> str | None:
    return current_organization_id()


def _company_by_id(company_id: str) -> dict[str, Any] | None:
    return next(
        (item for item in _read(COMPANIES_FILE) if str(item.get("id") or "") == str(company_id)),
        None,
    )


def _sector_by_id(sector_id: str) -> dict[str, Any] | None:
    return next(
        (item for item in _read(SECTORS_FILE) if str(item.get("id") or "") == str(sector_id)),
        None,
    )
...
def _agent_by_id(agent_id: str) -> dict[str, Any] | None:
    return next(
        (item for item in _read(AGENTS_FILE) if str(item.get("id") or "") == str(agent_id)),
        None,
    )


def _assert_company_access(company_id: str) -> dict[str, Any]:
    company = _company_by_id(company_id)
    if not company:
        raise ValueError("Empresa não encontrada.")
    organization_id = _current_organization()
    if organization_id:
        assert_organization_access(
            company,
            organization_id,
            resource="Empresa",
            company_is_tenant=False,
            allow_unscoped=True,
        )
    return company


def _assert_agent_access(agent_id: str) -> dict[str, Any]:
    agent = _agent_by_id(agent_id)
    if not agent:
        raise ValueError("Agente não encontrado.")
    organization_id = _current_organization()
    if organization_id and agent_id != "general":
        assert_organization_access(agent, organization_id, resource="Agente")
    return agent


def _resolve_resource_organization(explicit: Any = None) -> str | None:
    organization_id = normalize_organization_id(explicit)
    current = _current_organization()
    ...
    return organization_id or current


def _assert_organization_compatible(
    ...
    *,
    ...
    if organization_id and actual and actual != organization_id:
...
def list_companies() -> list[dict[str, Any]]:
    ...
    return [{**company, "color": company_color(company)} for company in companies]
...
def get_company(company_id: str) -> dict[str, Any] | None:
    ...
    result = dict(company)
    result["color"] = company_color(company)
    return result
...
def create_company(data: dict[str, Any]) -> dict[str, Any]:
    ...
    organization_id = _resolve_resource_organization(data.get("organization_id"))
    now = _now()
...
def update_company(company_id: str, data: dict[str, Any]) -> dict[str, Any]:
    current = _assert_company_access(company_id)
    requested_color = _validate_company_color(data["color"]) if "color" in data else None
    organization_id = _resolve_resource_organization(data.get("organization_id", current.get("organization_id")))
    ...
    def mutate_company(companies: list[dict[str, Any]]) -> dict[str, Any]:
        for company in companies:
            if company.get("id") == company_id:
                ...
                if organization_id:
                    company["organization_id"] = organization_id
                elif "organization_id" in company:
                    company.pop("organization_id", None)
...
def list_sectors(company_id: str | None = None) -> list[dict[str, Any]]:
    ...
    if organization_id:
        sectors = [
            ...
            if organization_matches_sector(sector, organization_id)
        ]
    if company_id:
        sectors = [s for s in sectors if s.get("company_id") == company_id]
    return sectors


def organization_matches_sector(sector: dict[str, Any], organization_id: str) -> bool:
    company = _company_by_id(str(sector.get("company_id") or ""))
    if not company:
        return False
    return bool(
        entity_organization_id(company, company_is_tenant=False) == organization_id
    )


def get_sector(sector_id: str) -> dict[str, Any] | None:
    sector = _sector_by_id(sector_id)
    if not sector:
        return None
    organization_id = _current_organization()
    if organization_id:
        company = _company_by_id(str(sector.get("company_id") or ""))
        if not company or entity_organization_id(company, company_is_tenant=False) != organization_id:
            raise PermissionError("Setor pertence a outra organization.")
    return dict(sector)
...
def create_sector(data: dict[str, Any]) -> dict[str, Any]:
    ...
    company = _assert_company_access(company_id)
    organization_id = _resolve_resource_organization(data.get("organization_id", company.get("organization_id")))
    now = _now()
    ...
    def append_sector(sectors: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if organization_id:
            sector["organization_id"] = organization_id
...
def update_sector(sector_id: str, data: dict[str, Any]) -> dict[str, Any]:
    current = get_sector(sector_id)
    if not current:
        raise ValueError("Setor não encontrado.")
    name = str(data.get("name", current.get("name")) or "").strip()
    if not name:
        raise ValueError("Nome do setor é obrigatório.")
    company_id = str(data.get("company_id", current.get("company_id")) or "").strip()
    company = _assert_company_access(company_id)
    organization_id = _resolve_resource_organization(
        data.get("organization_id", current.get("organization_id", company.get("organization_id")))
    )
    ...
    def mutate_sector(sectors: list[dict[str, Any]]) -> dict[str, Any]:
        for sector in sectors:
            if sector.get("id") == sector_id:
                ...
                if organization_id:
                    sector["organization_id"] = organization_id
                elif "organization_id" in sector:
                    sector.pop("organization_id", None)
...
def _assert_agent_organization_compatible(
    company_id: str | None,
    sector_id: str | None,
    organization_id: str | None,
) -> None:
    ...
    if company_id:
        ...
        company_organization = entity_organization_id(company, company_is_tenant=False) if company else None
        if company_organization and company_organization != organization_id:
            raise PermissionError("Empresa pertence a outra organization.")
    if sector_id:
        sector = get_sector(sector_id)
        sector_company_id = str((sector or {}).get("company_id") or "")
        sector_company = get_company(sector_company_id) if sector_company_id else None
        sector_organization = (
            entity_organization_id(sector_company, company_is_tenant=False)
            if sector_company
            else None
        )
        if sector_organization and sector_organization != organization_id:
            raise PermissionError("Setor pertence a outra organization.")


def list_agents() -> list[dict[str, Any]]:
    agents = _read(AGENTS_FILE)
    return filter_by_organization(agents, include_global=True)
...
def create_agent(data: dict[str, Any]) -> dict[str, Any]:
    ...
    _assert_agent_organization_compatible(company_id, sector_id, organization_id)
    if _current_organization() and not company_id and str(name).lower() != "general":
        raise ValueError("Agente precisa pertencer a uma empresa da organization atual.")
...
def update_agent(agent_id: str, data: dict[str, Any]) -> dict[str, Any]:
    current = _assert_agent_access(agent_id)
    current_organization = _current_organization()
    requested_organization = normalize_organization_id(data.get("organization_id"))
    if current_organization and requested_organization and requested_organization != current_organization:
        raise PermissionError("Agente foi informado para outra organization.")
    organization_id = requested_organization or current_organization or normalize_organization_id(current.get("organization_id"))
    ...
    def mutate_agent(agents: list[dict[str, Any]]) -> dict[str, Any]:
        for agent in agents:
            if agent.get("id") == agent_id:
                ...
                if organization_id:
                    agent["organization_id"] = organization_id
                elif "organization_id" in agent:
                    agent.pop("organization_id", None)
...
def _conversation_by_id(conversation_id: str) -> dict[str, Any] | None:
    return next(
        (item for item in _read(CONVERSATIONS_FILE) if item.get("id") == conversation_id),
        None,
    )


def _run_by_id(run_id: str) -> dict[str, Any] | None:
    return next((item for item in _read(RUNS_FILE) if item.get("id") == run_id), None)


def _agent_organization_id(agent_id: str) -> str | None:
    agent = _agent_by_id(agent_id)
...
def list_conversations(agent_id: str | None = None) -> list[dict[str, Any]]:
    ...
    conversations = filter_by_organization(conversations)
    return sorted(conversations, key=lambda c: c.get("updated_at", ""), reverse=True)


def get_conversation(conversation_id: str) -> dict[str, Any] | None:
    conversation = _conversation_by_id(conversation_id)
    if not conversation:
        return None
    organization_id = _current_organization()
    if organization_id:
        assert_organization_access(conversation, organization_id, resource="Conversa")
    return dict(conversation)


def create_conversation(agent_id: str, title: str | None = None) -> dict[str, Any]:
    agent = _assert_agent_access(agent_id)
    organization_id = _resolve_resource_organization(agent.get("organization_id"))
    conversations = _read(CONVERSATIONS_FILE)
    now = _now()
    conversation = {
        "id": uuid4().hex,
...
def append_conversation_message(
    ...
    *,
    ...
    for current in conversations:
        if current.get("id") != conversation_id:
            continue
        ...
        if role == "user" and (current.get("title") in {None, "", "Nova conversa"}):
            clean = re.sub(r"\s+", " ", content).strip()
            current["title"] = clean[:52] + ("…" if len(clean) > 52 else "")
...
def list_runs(agent_id: str | None = None) -> list[dict[str, Any]]:
    ...
    return sorted(filter_by_organization(runs), key=lambda r: r.get("started_at", ""), reverse=True)
...
def get_run(run_id: str) -> dict[str, Any] | None:
    ...
    organization_id = _current_organization()
    if organization_id:
        assert_organization_access(run, organization_id, resource="Run")
    return dict(run)
...
def start_run(
    ...
    *,
    ...
    if parent_run and organization_id:
        assert_organization_access(parent_run, organization_id, resource="Run ancestral")
...
def interrupt_stale_running_runs(reason: str = "Execução interrompida pela reinicialização do ACTIS GEN.") -> int:
    runs = filter_by_organization(_read(RUNS_FILE))
    changed = 0
    for run in runs:
