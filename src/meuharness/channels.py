...
def ensure_default_channel() -> None:
    with connect() as conn:
        ...
        now = _now()
        conn.execute(
            "INSERT INTO agent_channels(id,name,topic,scope_type,scope_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
            (uuid.uuid4().hex, "#general", "Diretrizes e decisões globais de longo prazo.", "global", None, now, now),
        )
...
def create_channel(data: dict[str, Any]) -> dict[str, Any]:
    ...
    with connect() as conn:
        conn.execute(
            "INSERT INTO agent_channels(id,name,topic,scope_type,scope_id,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
            (channel_id, name[:80], topic[:500], scope_type, scope_id, now, now),
        )
...
def create_channel_message(channel_id: str, data: dict[str, Any]) -> dict[str, Any]:
    ...
    with connect() as conn:
        conn.execute("""
            ...
            (id,channel_id,author_type,author_id,author_name,kind,content,pinned,long_term,status,created_at,updated_at)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            item["id"], channel_id, item["author_type"], item["author_id"], item["author_name"],
            kind, content, item["pinned"], item["long_term"], status, now, now,
        ))
        conn.execute("UPDATE agent_channels SET updated_at=? WHERE id=?", (now, channel_id))
    emit_event(
...
def delete_channel_message(message_id: str) -> bool:
    ...
    with connect() as conn:
        row = conn.execute(
            "SELECT channel_id FROM agent_channel_messages WHERE id=?", (message_id,)
        ).fetchone()
        if not row:
            return False
...
def delete_channel(channel_id: str) -> bool:
    init_channels_schema()
    with connect() as conn:
        cursor = conn.execute("DELETE FROM agent_channels WHERE id=? AND name<>'#general'", (channel_id,))
    return bool(cursor.rowcount)
