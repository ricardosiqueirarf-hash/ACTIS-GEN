...
SKILL_CATALOG: dict[str, dict[str, Any]] = {
    ...
    "colorglass.commercial-quotation": _skill(
        ...
        activation=["orçamento", "orcamento", "orçar", "orcar", "porta", "giro", "fixa", "deslizante", "perfil", "vidro", "espelho"],
        instructions=(
            "COLORGLASS COMMERCIAL QUOTATION: você é comercial, não formulário. Converse normalmente. "
            "Quando houver intenção real de orçamento, identifique a tipologia e consulte colorglass_quote_schema para obter o contrato JSON mínimo. "
            "Use a conversa como fonte dos valores: preencha mentalmente o JSON e pergunte SOMENTE campos de required ainda ausentes. "
            "Nunca exponha o JSON ao cliente, nunca peça campo já informado e nunca crie estado determinístico de coleta. "
            "profile deve ser composto como '<profile_code> <color>'. Campos optional usam os defaults do schema, salvo se o cliente especificar outro valor. "
            "Quando required estiver completo, use colorglass_quote_create_with_door. Se houver conjuntos diferentes no mesmo orçamento, crie o primeiro e use colorglass_quote_add_door_fast para os demais. "
            "Nunca estime preço. O valor oficial é o retornado pela tool. Se pdf_ready=true e pdf_path estiver disponível, envie o PDF por whatsapp_send_document com uma legenda curta e natural. "
            "Se a tool oficial exigir um dado adicional no preview, pergunte esse dado naturalmente e tente novamente. "
    ...
    "colorglass.finance": _skill(
        ...
        inherits=["colorglass.operations"], runtime_features=["colorglass.finance.local"], eager=True,
