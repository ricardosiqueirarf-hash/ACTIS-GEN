...
def _assert_connector_access(connector: dict[str, Any]) -> None:
...
def _resolve_connector_organization(data: dict[str, Any], current: dict[str, Any] | None) -> str | None:
    return resolve_inherited_organization_id(data.get("organization_id"), current)


def list_connectors() -> list[dict[str, Any]]:
    items = _ensure_defaults()
    return [dict(item) for item in filter_by_organization(items, include_global=True, include_unscoped=True)]


def get_connector(connector_id: str) -> dict[str, Any] | None:
    item = next((dict(value) for value in _ensure_defaults() if value.get("id") == connector_id), None)
    if item:
        _assert_connector_access(item)
    return item
...
def save_connector(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if current:
        _assert_connector_access(current)
    organization_id = _resolve_connector_organization(data, current)
...
def connector_visible_to_agent(connector: dict[str, Any], agent_id: str) -> bool:
    _assert_connector_access(connector)
    agent = get_agent(agent_id)
    connector_organization = normalize_organization_id(connector.get("organization_id"))
    agent_organization = normalize_organization_id((agent or {}).get("organization_id"))
    if connector_organization and agent_organization and connector_organization != agent_organization:
        return False
