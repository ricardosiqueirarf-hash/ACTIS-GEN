...
from meuharness.adapters.maf.observed_tool import clear_run_tool_trace, get_run_tool_trace, observe_native_tool
...
_SELF_VERIFYING_BROWSER_ACTION_TOOLS = {"colorglass_quote_create", "colorglass_quote_add_door"}
...
def _effective_tools(agent: dict[str, Any], history: list[dict[str, str]], prompt: str) -> list[str]:
    ...
    effective = normalize_tool_ids([*list(agent.get("tools") or []), *inherited])
...
def _effective_scopes(agent: dict[str, Any], effective: list[str]) -> set[str]:
    ...
    inherited = tools_for_skills(skill_ids)
    declared = normalize_tool_ids([*list(agent.get("tools") or []), *inherited])
...
def _runtime_tools(
    ...
    if contact_scope:
        ...
        native = build_company_tools(agent_id, run_id=run_id, env_file=env_file)
        native.extend(build_whatsapp_tools(agent_id, source=source, run_id=run_id,
                                      contact_scope=contact_scope, delivery_key=delivery_key))
    ...
    if "colorglass.quotation.native" in runtime_features:
        ...
        tools.extend(build_colorglass_quotation_tools(agent_id))
    if "harness_browser" in effective:
...
async def execute_agent(
    ...
    *,
    ...
    if (uses_computer or uses_browser) and settings.timeout_s < 300:
    ...
    ) + context_instructions(context_package) + channel_context_instructions(channel_items)
    ...
    connector_catalog = [] if whatsapp_contact else connector_runtime_catalog(agent_id)
    ...
    tools = _runtime_tools(agent_id, runtime_features, effective, scopes, env_file, settings.model, run["id"], source, whatsapp_contact, whatsapp_delivery_key)
