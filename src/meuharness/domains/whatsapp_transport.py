...
MESSAGE_SCAN_JS = r"""(() => {
  ...
  for(const row of rows){
    const probe=row.querySelector('[data-pre-plain-text]')?.getAttribute('data-pre-plain-text')||'';
    const m=probe.match(/^\[(\d{1,2}):(\d{2}),\s*(\d{1,2})\/(\d{1,2})\/(\d{2,4})\]/);
    if(!m) continue;
    const first=Number(m[3]),second=Number(m[4]);
    if(first>12&&second<=12){ dateOrder='DMY'; break; }
    if(second>12&&first<=12){ dateOrder='MDY'; break; }
  }
  for(const row of rows){
