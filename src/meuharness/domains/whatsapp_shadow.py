...
def ingest_messages(agent_id: str, contact_name: str,
    ...
    with connect() as conn:
        for message in messages:
            ...
            if was_inserted:
                ...
                if direction == "incoming":
                    incoming_inserted += 1
                    if effective_ts:
                        last_customer_at = max(last_customer_at or effective_ts, effective_ts)
                    if not timestamp or not conversation.get("last_message_at") or timestamp > conversation["last_message_at"]:
                        emitted.append({
                    ...
                    if effective_ts:
                        last_agent_at = max(last_agent_at or effective_ts, effective_ts)
            if timestamp and (newest_ts is None or timestamp > newest_ts):
                newest_ts, newest_external = timestamp, external_id
        unread_increment = len(emitted) if emit_inbound_events else 0
        conn.execute("""
            UPDATE whatsapp_conversations
            SET last_message_at=MAX(COALESCE(?,''),COALESCE(last_message_at,'')),
                last_synced_at=?, sync_status='healthy', updated_at=?,
                unread_count=unread_count+?,
                first_inbound_at=COALESCE(first_inbound_at,?),
                last_customer_at=MAX(COALESCE(?,''),COALESCE(last_customer_at,'')),
                last_agent_at=MAX(COALESCE(?,''),COALESCE(last_agent_at,''))
            WHERE id=?
        """, (newest_ts, now, now, unread_increment, last_customer_at,
              last_customer_at, last_agent_at, conversation["id"]))
        conn.execute("""INSERT INTO whatsapp_sync_state(conversation_id,last_external_id,last_message_ts,last_success_at,status,error,updated_at)
                     ...
                     (conversation["id"], newest_external, newest_ts, now, "healthy", None, now))
