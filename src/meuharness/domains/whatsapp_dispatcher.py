...
# Process-local cache for a chat that was already verified by phone/name.
# The cache is only reused while the visible header still matches, so a chat
# changed by another operation automatically invalidates the fast path.
_VERIFIED_ACTIVE_CHAT: dict[str, dict[str, str]] = {}
...
def _page_cache_key(page: dict[str, Any]) -> str:
    return str(page.get("id") or page.get("webSocketDebuggerUrl") or "")


def _active_chat_header(page: dict[str, Any]) -> str:
    value = _eval(page, "document.querySelector('#main header [data-testid=conversation-info-header]')?.innerText||document.querySelector('#main header')?.innerText||''")
    return str(value or "").split("\n", 1)[0].strip()


def _remember_verified_chat(page: dict[str, Any], contact_name: str, phone: str) -> None:
    key = _page_cache_key(page)
    header = _active_chat_header(page)
    if key and header:
        _VERIFIED_ACTIVE_CHAT[key] = {
            "contact_name": contact_name.strip().casefold(),
            "phone": normalize_phone(phone),
            "header": header,
        }


def _reuse_verified_chat(page: dict[str, Any], contact_name: str, phone: str) -> bool:
    cached = _VERIFIED_ACTIVE_CHAT.get(_page_cache_key(page))
    if not cached:
        return False
    wanted_name = contact_name.strip().casefold()
    wanted_phone = normalize_phone(phone)
    if cached.get("contact_name") != wanted_name or cached.get("phone") != wanted_phone:
        return False
    try:
        return bool(_composer_ready(page) and _active_chat_header(page) == cached.get("header"))
    except Exception:
        return False
...
def _open_chat(page: dict[str, Any], contact_name: str, external_id: str | None) -> None:
    ...
    if _reuse_verified_chat(page, contact_name, phone):
        return
    if phone:
        ...
        _remember_verified_chat(page, contact_name, phone)
        return
        ...
        try:
            ...
            if chooser and chooser.get("backendNodeId"):
                ...
                if object_id:
                    ...
                    detail = ((check.get("result") or {}).get("value") or {})
                    stage_detail = json.dumps(detail, ensure_ascii=False)
                    if detail.get("count") and not detail.get("connected") and inject_existing_document_input():
                        stage_method = "liveInputFallback"
...
def _send_and_verify(page: dict[str, Any], content: str) -> str:
    ...
    try:
        ...
        def verified() -> bool:
            ...
            return bool(new_found)
        ack_timeout = 45 if len(content) > 320 else 18
        if not _wait_until(verified, timeout=ack_timeout, interval=0.4):
...
def is_delegated_operator_notification(agent_id: str, contact_name: str, source: str) -> bool:
    """Allow a narrow internal notification path to the company's configured human operator."""
    if str(source or "") != "delegation":
        return False
    from meuharness.agents import get_agent, get_company

    agent = get_agent(str(agent_id or ""))
    if not agent or not agent.get("company_id"):
        return False
    company = get_company(str(agent.get("company_id") or ""))
    operator_contact = str((company or {}).get("operator_contact") or "").strip()
    return bool(operator_contact and operator_contact == str(contact_name or "").strip())
...
def _delivery_allowed(item: dict[str, Any]) -> tuple[bool, str]:
    ...
    operator_notification = is_delegated_operator_notification(
        str(item.get("agent_id") or ""), str(item.get("contact_name") or ""), source
    )
    if mode == "disabled":
    ...
    if automatic and mode != "autonomous" and not operator_notification:
        return False, f"automation_mode:{mode}"
    if automatic and not operator_notification:
...
def _dispatch_baileys(agent_id: str, outbox_id: str, max_attempts: int) -> dict[str, Any]:
    from meuharness.domains.whatsapp_baileys import health as baileys_health, send_outbox
    ...
    if not allowed:
        return block_outbox(outbox_id, reason) or existing
    state = baileys_health()
    if not state.get("connected"):
        return {**existing, "_fallback": True, "reason": "baileys_unavailable", "primary": state}
    item = claim_outbox_id(outbox_id, max_attempts=max_attempts)
    if not item or not item.pop("_claimed", False):
        return item or existing
    emit_event(
        "whatsapp.outbox.sending", agent_id=agent_id, outbox_id=outbox_id,
        conversation_id=item.get("conversation_id"), contact=item.get("contact_name"),
        transport="baileys",
    )
    try:
        result = send_outbox(item)
    except Exception as exc:  # noqa: BLE001 - pre-send client failure; no provider call completed
        released = release_outbox_for_fallback(outbox_id, "baileys_client_error:" + str(exc)) or item
        return {**released, "_fallback": True, "reason": "baileys_client_error"}
    if result.get("ok") and result.get("status") == "accepted" and result.get("message_id"):
        return complete_outbox(outbox_id, transport_ref="baileys:" + str(result["message_id"])) or item
    attempted = bool(result.get("attempted"))
    safe_fallback = bool(result.get("safe_fallback")) and not attempted
    error = str(result.get("error") or result.get("reason") or result.get("status") or "baileys_failed")
    if safe_fallback:
        released = release_outbox_for_fallback(outbox_id, error) or item
        return {**released, "_fallback": True, "reason": "baileys_safe_fallback", "primary": result}
    if attempted:
        return uncertain_outbox(outbox_id, "Baileys: " + error) or item
    return fail_outbox(outbox_id, "Baileys: " + error, max_attempts=max_attempts) or item


def _dispatch_web(agent_id: str, outbox_id: str, max_attempts: int) -> dict[str, Any]:
    item = outbox_item(outbox_id)
    if not item or item.get("agent_id") != agent_id:
        raise ValueError("Outbox item não encontrado para este agente")
    try:
        with browser_lease(agent_id, wait_seconds=8):
            result = _dispatch_locked(agent_id, outbox_id, max_attempts)
            if isinstance(result, dict):
                result = {**result, "transport": "web_fallback"}
            return result
    except BrowserBusyError:
        return {**item, "ok": False, "reason": "browser_busy", "transport": "web_fallback"}


def dispatch_outbox_item(agent_id: str, outbox_id: str, *, max_attempts: int = 3) -> dict[str, Any]:
    primary = _dispatch_baileys(agent_id, outbox_id, max_attempts)
    if primary.get("_fallback"):
        return _dispatch_web(agent_id, outbox_id, max_attempts)
    return {**primary, "transport": "baileys"} if isinstance(primary, dict) else primary
...
def dispatch_once(agent_id: str, *, limit: int = 3, max_attempts: int = 3) -> list[dict[str, Any]]:
    ...
    return [dispatch_outbox_item(agent_id, str(row["id"]), max_attempts=max_attempts)
            for row in items[:max(1, min(limit, 20))]]


def open_session(agent_id: str) -> dict[str, Any]:
    from meuharness.domains.whatsapp_baileys import health as baileys_health

    primary = baileys_health()
    if primary.get("connected"):
        return {"ok": True, "connected": True, "transport": "baileys", "primary": primary}
    try:
        with browser_lease(agent_id, wait_seconds=8):
            page = _page(agent_id)
            connected = bool(page and _authenticated(page))
            return {
                "ok": connected, "connected": connected, "transport": "web_fallback",
                "reason": "connected" if connected else "authentication_required",
                "primary": primary,
            }
    except BrowserBusyError:
        return {"ok": False, "reason": "browser_busy", "transport": "web_fallback", "primary": primary}
