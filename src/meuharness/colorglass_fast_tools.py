...
QUOTE_SCHEMAS = {
    "giro": {
        ...
        "price_sensitive": ["puller", "puller_size_mm", "additional_value"],
        "optional": {
        ...
        "technical": {
            "confirm_existing": ["width_mm", "height_mm", "profile_code", "color", "glass"],
            "required": ["environment", "hinges", "hinge_side", "hinge_heights_mm", "drilling_position", "puller"],
            "conditional": {
                "puller_side": "obrigatório quando houver puxador",
                "puller_size_mm": "obrigatório quando o puxador tiver medida configurável",
            },
            "optional": ["production_note"],
        },
    ...
    "deslizante": {
        ...
        "required": ["quantity", "width_mm", "height_mm", "profile_code", "color", "glass"],
        "price_sensitive": [
            "system", "upper_track", "lower_track", "upper_track_gap_mm", "lower_track_gap_mm",
            "puller", "puller_size_mm", "additional_value",
        ],
        "optional": {
        ...
        "technical": {
            "confirm_existing": ["width_mm", "height_mm", "profile_code", "color", "glass", "system", "upper_track", "lower_track"],
            "required": ["environment", "upper_track_gap_mm", "lower_track_gap_mm", "puller"],
            "conditional": {
                "puller_side": "obrigatório quando houver puxador",
                "puller_size_mm": "obrigatório quando o puxador tiver medida configurável",
            },
        ...
        "compose": {"profile": "<profile_code> <color>"},
        "note": "Na fase comercial, sistema/trilhos/vãos entram apenas quando fizerem parte do preço. Na fase técnica devem ser confirmados para produção.",
    },
}


def _record_commercial_quote_state(agent_id: str, contact_name: str, spec: dict[str, Any], result: dict[str, Any]) -> None:
    """Persist only commercial truth after pricing; never promote technical defaults."""
    try:
        from meuharness.domains.whatsapp_shadow import get_conversation_by_contact
        if not get_conversation_by_contact(agent_id, contact_name):
            return
        ...
        state = get_quote_state(agent_id, contact_name)
        existing = dict(state.get("fields") or {})
        profile = str(spec.get("perfil") or "").strip()
        profile_code_match = re.search(r"\b(1036|070|066|3446|3545|9700)\b", profile)
        color_match = re.search(r"\b(prata|dourad[oa]|pret[oa]|bronze|inox)\b", profile, re.I)
        commercial_fields = {
            "quantity": int(spec.get("quantidade") or 0),
            "typology": str(spec.get("tipo") or ""),
            "width_mm": int(spec.get("largura_mm") or 0),
            "height_mm": int(spec.get("altura_mm") or 0),
            "profile": profile,
            "glass": str(spec.get("vidro") or ""),
        }
        if profile_code_match:
            commercial_fields["profile_code"] = profile_code_match.group(1)
        if color_match:
            commercial_fields["color"] = color_match.group(1).lower()
        # Preserve facts explicitly learned in chat, but never copy _door_spec's
        # hinge/puller/production defaults into technical confirmation state.
        merged = dict(existing)
        merged.update({k: v for k, v in commercial_fields.items() if v not in {"", 0}})
        state["fields"] = merged
        state["commercial_snapshot"] = dict(commercial_fields)
        state["quote_phase"] = "commercial"
        state["status"] = "quoted"
        state["order_ref"] = str(result.get("numero_pedido") or state.get("order_ref") or "")
        state["last_agent_action"] = "commercial_quote_created"
        state["missing_fields"] = commercial_missing_fields(state)
        save_quote_state(agent_id, contact_name, state)
    except Exception:
        # Pricing success must not be downgraded because auxiliary phase tracking failed.
        return


def build_colorglass_fast_tools(agent_id: str, *, run_id: str | None = None) -> list[object]:
    def colorglass_quote_schema(
        door_type: Annotated[str, "Tipologia: giro, fixa ou deslizante"],
        phase: Annotated[str, "Fase: commercial ou technical"] = "commercial",
    ) -> str:
        """Retorne o contrato da fase correta sem misturar preço comercial com ficha de produção."""
        ...
        stage = str(phase or "commercial").strip().lower()
        if stage in {"technical", "tecnico", "técnico", "production", "producao", "produção"}:
            return json.dumps({
                "ok": True,
                "phase": "technical",
                "schema": {"tipologia": key, **dict(schema.get("technical") or {})},
                "rule": "Dados técnicos são confirmação de produção. Nunca trate defaults usados no orçamento como confirmados.",
            }, ensure_ascii=False)
        commercial = {k: v for k, v in schema.items() if k != "technical"}
        return json.dumps({
            "ok": True,
            "phase": "commercial",
            "schema": commercial,
            "rule": "Colete somente o necessário para preço. Não peça dados exclusivos de fabricação nesta fase.",
        }, ensure_ascii=False)
    ...
    def colorglass_quote_create_with_door(
        ...
        if result.get("ok"):
            _record_commercial_quote_state(agent_id, name, spec, result)
