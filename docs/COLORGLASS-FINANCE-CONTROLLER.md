...
## Skill operacional de sincronização bancária

Skill: `colorglass.finance.bank-sync`

Arquivo: `src/meuharness/skill_assets/colorglass-finance-bank-sync/SKILL.md`

A skill é herdada por `colorglass.finance` com progressive disclosure e só carrega o
playbook completo em Runs de banco/Unicred/extrato/DDA/sincronização.
...
Contrato:
- login sempre humano;
- uma única camada autorizada para operar a sessão: Unicred Bank Bridge;
- sync completo exige extrato + DDA + persistência confirmada;
- DDA parcial nunca substitui o último snapshot completo;
- DDA completo significa visíveis + ocultos + todas as páginas + todas as janelas;
- se extrato e DDA já foram sincronizados com sucesso no dia, não reabrir o banco automaticamente; repetir apenas por pedido explícito/retry;
- nenhuma tool da skill movimenta dinheiro.
